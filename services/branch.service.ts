...
export const BranchService = {
  list: async (token: string, query: string): Promise<PageResonse<Branch[]>> => {
    try {
      ...
      return {
        ...data,
        data: data.data.map((item: any): Branch => ({
          id: item.id,
          name: item.name,
  ...
  update: async (token: string, id: string, branch: unknown): Promise<Branch> => {
    try {
