...
export const getLoanColumns = ({
  ...
  },
  {
    ...
    header: "Khách Hàng",
    cell: ({ row }) => <span className="font-medium">{row.original.customer?.fullName || "N/A"}</span>,
  },
  {
    ...
    cell: ({ row }) => (
      <span className="font-bold text-primary">
        {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(row.original.totalLoan)}
      </span>
    ),
  },
  {
    ...
    header: "Ngày Tạo",
    cell: ({ row }) => new Date(row.original.loanDate).toLocaleDateString("vi-VN"),
  },
  {
