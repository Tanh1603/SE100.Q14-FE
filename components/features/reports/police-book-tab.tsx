...
import { DailyLogEntry, EnrichedDailyLogEntry, DailyLogSummary } from "@/types/report";
...
const PoliceBookTab = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const enrichLoans = async (loans: DailyLogEntry[]): Promise<EnrichedDailyLogEntry[]> => {
            return await Promise.all(loans.map(async (log) => {
                try {
                    // This is "heavy", but necessary for the report compliance if backend doesn't provide it
                    const loanDetail = await LoanService.getLoanById(log.contractId);
                    return {
                        ...log,
                    ...
                    console.error(`Failed to fetch details for loan ${log.contractId}`, e);
  ...
  const suspiciousItems = allLoans.filter(l => l.collateralDescription.toLowerCase().includes("nghi vấn")); // Mock logic
        ...
        <div className="print:hidden">
            ...
            <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
                ...
                <div className="text-sm text-gray-500">
                    Tổng giao dịch: {data.summary.totalNewLoans + data.summary.totalClosedLoans}
                </div>
                </div>
                <Table>
                <TableHeader>
                    ...
                    {allLoans.length > 0 ? (
                        ...
                        <TableRow key={log.contractId}>
                            ...
                            <TableCell className="text-center">
                                <span className={`px-2 py-1 rounded text-[10px] font-bold ${log.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                                    {log.status}
                                </span>
                            </TableCell>
                        </TableRow>
