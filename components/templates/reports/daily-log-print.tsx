...
export const DailyLogPrint = React.forwardRef<HTMLDivElement, DailyLogPrintProps>(
  ({ data, storeName, storeAddress }, ref) => {
    ...
    return (
      <div ref={ref} className="p-8 bg-white text-black font-serif text-sm w-full landscape:w-auto">
        ...
        <div className="mb-6">
          <p>
            <strong>Tên cơ sở kinh doanh:</strong> {storeName || "...................."}
          </p>
          <p>
            <strong>Địa điểm kinh doanh:</strong> {storeAddress || "...................."}
          </p>
          <p>
            <strong>Ngày báo cáo:</strong> {new Date(data.date).toLocaleDateString("vi-VN")}
          </p>
        </div>
        ...
        <table className="w-full border-collapse border border-black text-xs">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="border border-black p-1 text-center w-24">Ngày nhận cầm cố</th>
              <th className="border border-black p-1 text-center">Họ và tên khách hàng</th>
              <th className="border border-black p-1 text-center w-24">Số CMND/ CCCD/ Hộ chiếu</th>
              <th className="border border-black p-1 text-center">Địa chỉ thường trú</th>
              <th className="border border-black p-1 text-center">Loại tài sản, đặc điểm, số máy/khung</th>
              <th className="border border-black p-1 text-center">Mã HĐ</th>
              <th className="border border-black p-1 text-center">Số tiền cầm cố (VNĐ)</th>
              <th className="border border-black p-1 text-center w-16">Lãi suất (%)</th>
              <th className="border border-black p-1 text-center w-16">Thời hạn (tháng)</th>
              <th className="border border-black p-1 text-center w-24">Ngày trả/ thanh lý</th>
            ...
            {allLoans.length > 0 ? (
              allLoans.map((loan, index) => (
                <tr key={loan.contractId}>
                  ...
                  <td className="border border-black p-1 text-center">{loan.nationalId}</td>
                  <td className="border border-black p-1">{loan.address}</td>
                  <td className="border border-black p-1">{loan.collateralDescription}</td>
                  <td className="border border-black p-1 text-center font-bold">{loan.loanCode}</td>
                  ...
                  <td className="border border-black p-1 text-center">
                    {loan.closedDate ? new Date(loan.closedDate).toLocaleDateString("vi-VN") : ""}
                  </td>
                  <td className="border border-black p-1 text-center">
                    {loan.status}
                  </td>
              ...
              <tr>
                <td colSpan={12} className="border border-black p-4 text-center italic">
